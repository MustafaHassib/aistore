...
- **Localised Open Graph image** per locale, served as a static file from `public/og/`.
  Dynamic generation with `next/og` was implemented and then removed: Satori cannot shape
  Arabic, failing on GSUB `lookupType 5 / substFormat 3` with both its default font and a
  vendored Noto Kufi Arabic. Rather than ship an English-only card or a broken Arabic one,
  both locales point at replaceable 1200x630 placeholders.
...
- Designed Open Graph share cards for `public/og/ar.png` and `public/og/en.png` (1200x630)
